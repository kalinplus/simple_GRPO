...
|---|---|---|---|
| ① | `grpo_vllm_one.py:82-113` | **GRPO loss**，全文核心 | `GRPO_step()`：advantage、ratio、clip、KL，20 行搞定 |
| ② | `grpo_vllm_one.py:66-78` | `get_per_token_logps` | per-token log 概率的计算方式 |
| ③ | `grpo_vllm_one.py:163-175` | Reward 函数 | `reward_correct` + `reward_format`，理解 reward 怎么来的 |
| ④ | `grpo_vllm_one.py:116-247` | `gen_worker`：生成 Worker | vLLM 生成 → 计算 reward → 组装 batch → POST 给 ref_server |
...
| ⑦ | `grpo_vllm_one.py:12-23` | 硬编码参数 | `beta`、`clip_param`、`gen_update_steps` 等核心超参 |
| ⑧ | `grpo_vllm_one.py:250-333` | **主训练循环** | `deepspeed` 初始化、三进程启动、训练+保存+同步 |
...
### 3.2 GRPO loss 逐行拆解 —— `grpo_vllm_one.py:82-113`
...
### 3.3 Reward 函数 —— `grpo_vllm_one.py:163-175`
...
### 3.4 gen_worker 的关键逻辑 —— `grpo_vllm_one.py:116-247`
...
### 3.7 训练主循环 —— `grpo_vllm_one.py:250-333`
...
```
simple_GRPO/
  grpo_vllm_one.py          # ← 主 GRPO 实现（~334 行），先读这个
